function [tprAtWP,auc,fpr,tpr] = fastROC(labels,scores,plot_flag, plotStyle, plotShades, plotMean,legendNames)
...
%         (default FALSE).
%         plotMean: indicates wether we plot the mean value or K folds
%         predictions
%
...
if plot_flag==1
    if(plotMean)
         x = semilogx(fpr,tpr,plotStyle,'LineWidth',2);
        
    else
       y1=  semilogx(fpr,tpr,plotStyle,'color',plotShades) ; 
...
tprAtWP = trapz(logVals , tpr(intervalIdxs) ) ;
     ...
     if(plotMean)
            ...
            legendName = sprintf('%s: %.3f', 'Sigm', tprAtWP);
            legend(x, legendName, 'Location', 'NorthWest' );
     end
